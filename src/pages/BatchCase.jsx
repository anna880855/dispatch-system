...
function CodeRow({ row, index, region, units, getCurrentRotUnit, onChange, onRemove }) {
  ...
  function setCode(v) {
    onChange({ ...row, codeType: v, isRotating: false, unit: '', units: [], da01Count: 1, referralReason: '', rejectReason: '' });
  }
  ...
  return (
    <div style={{ background: C.bg, borderRadius: 14, padding: '18px 20px', border: `1px solid ${C.border}`, marginBottom: 12, position: 'relative' }}>
      {/* 行號 + 刪除 */}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
        <span style={{ background: C.primary, color: '#fff', borderRadius: 20, padding: '2px 10px', fontSize: 12, fontWeight: 600 }}>
          碼別 {index + 1}
        </span>
        <button onClick={onRemove} style={{ background: 'none', border: 'none', color: C.muted, cursor: 'pointer', fontSize: 18, padding: 4 }}>✕</button>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px 20px' }}>
        ...
        {/* 派案單位 */}
        <FormField label={row.isRotating ? '派案單位（輪派自動帶入）' : unitList.length > 0 ? '派案單位（可多選）' : '派案單位'} fullWidth>
          {row.isRotating ? (
            rotInfo ? (
...
export default function BatchCase({ setPage }) {
  ...
  function removeRow(idx) { if (rows.length === 1) return; setRows(rs => rs.filter((_, i) => i !== idx)); }
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      for (const row of rows) {
        ...
        let finalReferral = row.referralReason === '其他' && row.referralReasonOther ? `其他：${row.referralReasonOther}` : row.referralReason;
        let finalReject = row.rejectReason === '其他' && row.rejectReasonOther ? `其他：${row.rejectReasonOther}` : row.rejectReason;

        for (const u of unitsList) {
          await addCase({
            ...base, codeType: row.codeType, isRotating: row.isRotating,
            unit: u, status: row.status, caseType: base.caseType,
  ...
  return (
    <div>
      <PageHeader title="批次派案" subtitle="填一次個案資料，一次新增多個碼別" />
      ...
      {/* 碼別列 */}
      <div style={{ fontWeight: 600, fontSize: 14, color: C.text, marginBottom: 12, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <span>📌 服務碼別（共 {rows.length} 個）</span>
        <BtnSmall onClick={addRow} style={{ background: C.primary, color: '#fff', border: 'none' }}>＋ 新增碼別</BtnSmall>
      </div>
      ...
      <div style={{ display: 'flex', gap: 12, marginTop: 8 }}>
        <BtnPrimary style={{ flex: 1 }} onClick={handleSubmit} disabled={saving}>
          {saving ? '儲存中…' : `儲存全部派案（${rows.length} 個碼別）`}
        </BtnPrimary>
